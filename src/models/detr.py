...
class DETR(nn.Module):
    ...
    def forward(self, samples):  # : NestedTensor
        ...
        """
        samples = self.init_conv(samples)
        ...
        hs = self.transformer(
            self.input_proj(src), None, self.query_embed.weight, pos[-1]
        )[0]
...
class SetCriterion(nn.Module):
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes):  # TODO ajouter ça
    ...
    def forward(self, outputs, targets, negative_sample=False):
        ...
        if negative_sample:
            losses = {
                ...
                for loss in ["loss_bbox", "loss_giou", "loss_ce", "cardinality_error"]
            }
            losses["loss_neg_ce"] = (
                -torch.log(outputs["pred_logits"].softmax(-1)[..., 0])
            ).mean()
            return losses
...
pretr = {
    "resnet101": "detr-r101-dc5-a2e86def.pth",
    "resnet50": "detr-r50-dc5-f0fb7ef5.pth",
    "resnet50_light": "detr-r50-e632da11.pth",
}


def load_weights(args, model, path=None, train=True):
    ...
    state_dict = {
        k: v
        for k, v in state_dict[weight_key].items()
        if k in model_dict and not np.array([e in k for e in exclude]).any()
    }
