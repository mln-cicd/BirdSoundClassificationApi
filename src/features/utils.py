...
def dir_convert_mp32wav(directory, keep_file=False):
    ...
    res = np.array(
        [
            file_convert_mp32wav(os.path.join(directory, f), keep_file=keep_file)
...
def read_txt_file(file, extra_str_label=""):
    ...
    df = (
        df.loc[df["line_type"] == 0]
        .merge(df.loc[df["line_type"] == 1], on="id")
        .dropna()
        .rename(
            columns={
                "0_x": "t_start",
...
def create_label_dataset(
    ...
    dict_dir = r""
    with open(os.path.join(dict_dir, "bird_dict.json")) as f:
    ...
    if is_csv:
        ...
        df_list = [
            read_txt_file(os.path.join(annot_dir, f), extra_str_label=extra_str_label)
    ...
    labels = (
        labels.sort_values("f_delta", ascending=False)
        .drop_duplicates(["filename", "t_start", "species"])
        .sort_values(["filename", "t_start"])
    )
    del labels["f_delta"]

    # Clean species
    labels["species"] = labels["species"].map(
        lambda x: replacements[x] if x in replacements.keys() else x
    )

    # Assign species to id
    labels["bird_id"] = labels["species"].map(
        lambda x: birds_dict[x] if x in birds_dict.keys() else np.nan
    )
    ...
    for filep in filesp:
        ...
        if len(annot_img) > 0:
            ...
            for j, bbox in enumerate(bboxes):
                ...
                x_labels = [
                    int(1000 * (x + fileidx * 819) * pix_precision_x) / 1000
                    for x in ax.get_xticks()
                ]
                ax.yaxis.set_major_locator(
                    mticker.FixedLocator(ax.get_yticks().tolist())
                )
                ax.xaxis.set_major_locator(
                    mticker.FixedLocator(ax.get_xticks().tolist())
                )
