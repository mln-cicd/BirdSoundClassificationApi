...
ornithos = {
    "NidalIssa": {"extra_label": ""},
    "KevinLeveque": {"extra_label": ""},
    "HerveRenaudineau": {"extra_label": ""},
    "GuillaumeBigayon": {"extra_label": ""},
    "GhislainRiou": {"extra_label": ""},
    "GaëtanMineau": {"extra_label": ""},
    "FredericCazaban": {"extra_label": ""},
    "ChristopheMercier": {"extra_label": ""},
    "AymericMousseau": {"extra_label": "amousseau_"},
    "AdrienPajot": {"extra_label": ""},
    "WillyRaitiere": {"extra_label": "willyraitiere_"},
    "MaxencePajot": {"extra_label": "Piste de marqueur"},
    "MathurinAubry": {"extra_label": ""},
    "LionelManceau": {"extra_label": ""},
    "mediae": {"extra_label": ""},
}
...
def prepare_dataset(
    ...
    extra_str_label = (
        ornithos[top_dir]["extra_label"] if top_dir in ornithos.keys() else ""
    )
    if audio_format != "":
        audio_files = glob.glob(os.path.joint(directory, "audio", f"*.{audio_format}"))
    else:
        audio_files = glob.glob(os.path.join(directory, "audio", "*.wav")) + glob.glob(
            os.path.join(directory, "audio", "*.mp3")
        )
    if annotations:
        labels = create_label_dataset(
            directory,
            extra_str_label=extra_str_label,
            suppress_unID=True,
            is_csv=top_dir == "mediae",
        )
    else:
        labels = None
    ...
    for file in audio_files:
        ...
        out_pos_dir = os.path.join(
            out_directory,
            "positive_files",
            top_dir + "__" + fp.filename.replace("#", "__"),
        )
        out_neg_dir = os.path.join(
            out_directory,
            "negative_files",
            top_dir + "__" + fp.filename.replace("#", "__"),
        )
        ...
        img_db, annotations = fp.process_file(
            freq_accuracy=freq_accuracy,
            dt=dt,
            overlap_spectro=overlap_spectro,
            w_pix=w_pix,
        )
        if img_db is None:
            continue
        if annotations is None:
            ...
            n_img = len(
                img_db
            )  # one case, we don't expect long noise file for dataset building
            ...
            pos_idx = np.concatenate(
                [idx + inc for idx, inc in zip(pos_idx, lengths)]
            ).astype(int)
        ...
        if len(pos_idx) > 0:
            ...
            annotations.to_csv(
                os.path.join(out_pos_dir, "annotations.csv"), sep=";", index=False
            )
        ...
        for i in range(n_img):
            if type(img_db[0]) == list:
                ...
                img = img_db[i]
            file_idx = (
                "__".join([top_dir, fp.filename.replace("#", "__"), format(i, "05d")])
                + ".png"
            )
...
class File_Processor:
    ...
    def __init__(self, filepath, extra_str_label="", labels=None):
        ...
        self.filename = (
            ...
            .replace("." + self.ext, "")
            .replace(extra_str_label, "")
        )
        self.filepath = filepath

    def process_file(
        self, freq_accuracy=33.3, dt=0.003, overlap_spectro=0.2, w_pix=1024
    ):
    ...
    def load(self):
        ...
        if sr != self.FREQ:
            ...
            if self.ext == "wav":
                command = (
                    "ffmpeg -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le -ar 44100 %s"
                    % (norm_filename, temp_f)
                )
            else:
    ...
    def process_long_file(self, data, freq_accuracy, dt, overlap_spectro):
        ...
        if len(data) > max_l:
            ...
            for k in range(int(len(data) / max_l) + 1):
                ...
                if self.labels is not None:
                    labels = self.labels.loc[
                        self.labels["filename"] == self.filename
                    ].copy()
                    ...
                    labels["filename"] = f"temp{k!s}"
                else:
                    labels = None
                fp = File_Processor(f"temp{k!s}.{self.ext}", "", labels)
                img_db_inc, annotations_inc = fp.process_file(
                    freq_accuracy=freq_accuracy,
                    dt=dt,
                    overlap_spectro=overlap_spectro,
                    w_pix=self.W_PIX,
    ...
    def split_power_spec(self, log_power_spec):
        ...
        img_db = []
        for k in range(
            max(1, int(1 + np.ceil((max_l - self.W_PIX) / self.HOP_SPECTRO)))
        ):
            ...
            if next_bin:
                img_db.append(
                    np.concatenate(
                        [
                            log_power_spec[s_bin][:, s_bin_idx:],
                            log_power_spec[e_bin][:, :e_bin_idx],
                        ],
                        axis=1,
                    )
                )
        ...
        if img_db[-1].shape[-1] < self.W_PIX:
            if (self.labels is not None) and len(
                self.labels.loc[self.labels["filename"] == self.filename]
            ) > 0:
                max_pix = int(
                    self.labels.loc[
                        self.labels["filename"] == self.filename, "t_end"
                    ].max()
                    / self.DT
                )
            else:
    ...
    def merge_and_filter_labels(self, img_db):
        ...
        img_coord = [
            (i * self.HOP_SPECTRO, i * self.HOP_SPECTRO + self.W_PIX - 1)
        ...
        for ex_label, new_label in zip(["f_start", "f_end"], ["y_1", "y_2"]):
            labels_[new_label] = (
                (
                    labels_[ex_label].clip(lower=self.LOW_FREQ, upper=self.HIGH_FREQ)
                    - self.LOW_FREQ
                )
                / self.FREQ_ACCURACY
            ).astype(int)
        ...
        labels_ = labels_[coord + ["w", "h", "joint", "bird_id"]].merge(
            img_coord, on="joint"
        )
        labels_ = labels_.loc[
            (labels_["x_1"].between(labels_["start"], labels_["end"]))
            | (labels_["x_2"].between(labels_["start"], labels_["end"]))
            | (labels_["x_1"].lt(labels_["start"]) & labels_["x_2"].gt(labels_["end"]))
        ]

        # Supress bbox with too small intersection with spectrogram
        labels_["inside"] = (
            labels_[["x_2", "end"]].min(axis=1)
            - labels_[["x_1", "start"]].max(axis=1)
            + 1
        )
        ...
        labels_["coord"] = [
            (x_1, y_1, x_2, y_2)
            for (x_1, y_1, x_2, y_2) in zip(
                labels_["x_1"], labels_["y_1"], labels_["x_2"], labels_["y_2"]
            )
        ]

        # Delete negative samples if they appear in a positive image
        labels_ = labels_.merge(
            labels_.loc[labels_["bird_id"] != -1]
            .groupby("index")
            .size()
            .reset_index()
        ...
        # One row per img
        labels_ = labels_.groupby("index", as_index=False).agg(
            {"coord": lambda x: x.tolist(), "bird_id": lambda x: x.tolist()}
        )
