...
class Img_dataset(Dataset):
    def __init__(self, dataset_path, transform=False):
        ...
        for f in os.listdir(os.path.join(self.ds_p, "positive_files")):
            self.positive_files.extend(
                [
                    os.path.basename(img)
                    for img in glob.glob(
                        os.path.join(self.ds_p, "positive_files", f) + "/*.png"
                    )
                ]
            )
        self.negative_files = []
        for f in os.listdir(os.path.join(self.ds_p, "negative_files")):
            self.negative_files.extend(
                [
                    os.path.basename(img)
                    for img in glob.glob(
                        os.path.join(self.ds_p, "negative_files", f) + "/*.png"
                    )
                ]
            )
        self.hard_negative_files = []
        for f in os.listdir(os.path.join(self.ds_p, "hard_neg")):
            self.hard_negative_files.extend(
                [
                    os.path.basename(img)
                    for img in glob.glob(
                        os.path.join(self.ds_p, "hard_neg", f) + "/*.png"
                    )
                ]
            )
    ...
    def __getitem__(self, idx):
        ...
        bboxes, bird_ids = annot.loc[
            annot["index"] == int(fileidx), ["coord", "bird_id"]
        ].values[0]
        ...
        neg_img = imageio.imread(
            os.path.join(self.ds_p, "negative_files", neg_file, negp)
        )
        ...
        if self.transform:
            std = img.std().item()
            noise = torch.clamp(
                torch.randn(img.shape).mul_(img.std().item() / 2), min=-0.5, max=0.5
            )
            # Random Gain
            ...
            if bool_transform[0] == 1:
                ...
                hard_neg_img = imageio.imread(
                    os.path.join(self.ds_p, "hard_neg", hard_neg_file, hard_negp)
                )
...
def atm_abs_coeff(T, h, f2):
    ...
    Fr_N = ((T_0 / T) ** 0.5) * (
        9 + 280 * h * np.exp(-4.17 * (-1 + (T_0 / T) ** (1 / 3)))
    )
    alpha = (
        869
        * f2
        * (
            ...
            * (
                ...
                + 0.1068 * np.exp(-3352 / T) / (Fr_N + f2 / Fr_N)
            )
        )
    )
