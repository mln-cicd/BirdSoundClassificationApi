...
def download_request(args):
    ...
    response = requests.get(
        "https://www.xeno-canto.org/api/2/recordings", params=parameters
    )
    js = response.json()
    ...
    continue_bool = input(
        "Continue? [y] / [n] / or type the number of files you want to download: "
    )
    try:
    ...
    for i in tqdm(indexes):
        ...
        urllib.request.urlretrieve(
            recording["file"], filename=os.path.join(write_path, filename)
        )
...
def dir_convert_mp32wav(directory, keep_file=False):
    ...
    res = np.array(
        [
            file_convert_mp32wav(mp3_file, keep_file=keep_file)
            for mp3_file in glob.glob(directory + "/*.mp3")
        ]
    ).sum(axis=0)
...
def download_species_ids(species, ids, out_dirp):
    ...
    while processed_files < required_files:
        page_number += 1
        response = requests.get(
            f"https://xeno-canto.org/api/2/recordings?query={species}&page={page_number!s}"
        )
        js = response.json()
        if "error" in js.keys():
            break
        ...
        if len(recordings) > 0:
            for j, recording in enumerate(recordings):
                ...
                if not os.path.exists(os.path.join(out_dirp, filename + ".mp3")):
                    urllib.request.urlretrieve(
                        recording["file"],
                        filename=os.path.join(out_dirp, filename + ".mp3"),
                    )
            processed_files += j + 1
...
def main():
    ...
    parser.add_argument(
        "-t", "--sound_type", type=str, help="Which type of sound (call, song, ...)"
    )
